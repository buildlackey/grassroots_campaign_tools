...
exports.PreferenceSvc = void 0;
class PreferenceSvc {
    constructor(docProps, userProps) {
        this.docProps = docProps;
        this.userProps = userProps;
    }
    static toBool(v, fallback) {
        if (v == null)
            return !!fallback;
    ...
    savePreferences(prefs, columnNames) {
        var _a, _b, _c;
        Logger.log("📥 [PreferenceSvc.savePreferences] called with prefs=%s, columns=%s", JSON.stringify(prefs), JSON.stringify(columnNames));
        const current = this.getPreferences();
        Logger.log("🔎 Current prefs: %s", JSON.stringify(current));
        const updatedPrefs = {
            sheetTabName: (_a = prefs.sheetTabName) !== null && _a !== void 0 ? _a : current.sheetTabName,
            addressColumn: (_b = prefs.addressColumn) !== null && _b !== void 0 ? _b : current.addressColumn,
            mapsApiKey: (_c = prefs.mapsApiKey) !== null && _c !== void 0 ? _c : current.mapsApiKey,
        ...
        if (!(callHasOnlyKey || callHasFullSet)) {
            ...
            throw new Error('Invalid preferences: provide either only {mapsApiKey}, or provide {mapsApiKey, sheetTabName, addressColumn, showLatLong, debug} plus columnNames[].');
        }
        ...
        if (callHasFullSet) {
            ...
            if (idx === -1)
                throw new Error('addressColumn must be one of columnNames.');
            Logger.log("💾 Storing doc properties: sheetTab=%s addrCol=%s showLatLong=%s colOffset=%s dbg=%s", updatedPrefs.sheetTabName, updatedPrefs.addressColumn, updatedPrefs.showLatLong, updatedPrefs.debug, idx);
    ...
    static create() {
        return new PreferenceSvc(PropertiesService.getDocumentProperties(), PropertiesService.getUserProperties());
    }
}
exports.PreferenceSvc = PreferenceSvc;
PreferenceSvc.DOC_KEYS = {
...
PreferenceSvc.USER_KEYS = {
    MAPS_KEY: 'prefs.mapsApiKey',
    DEBUG: 'prefs.debug',
};
// === Namespace exposure only (no global function shims) ===
globalThis.CAMPAIGN_TOOLS = globalThis.CAMPAIGN_TOOLS || {};
globalThis.CAMPAIGN_TOOLS.PreferenceSvc = PreferenceSvc;
